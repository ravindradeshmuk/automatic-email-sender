...
const EmailTemplate = () => {
  const [selectedSection, setSelectedSection] = useState('Internal Stakeholders');
  ...
  const statuses = ['Planned', 'Started', 'Inprogress', 'Completed', 'Delay', 'Disregard'];
  ...
  useEffect(() => {
    if (editClicked) {
      const url = `https://autoapi.cardzpay.com/client/template/api/templates?sections=${encodeURIComponent(selectedSection)}&templateName=${encodeURIComponent(selectedStatus)}`;
      fetch(url)
  ...
  const saveContent = () => {
    ...
    fetch(`https://autoapi.cardzpay.com/client/template/api/template/${currentTemplateId}`, {
      method: 'PUT',
    ...
    <div className="email-template-container" style={{ marginTop: '120px'}}>
      <div className="section-selection">
        ...
        <div className="template-section">
          ...
          <Button variant="contained" onClick={() => setEditClicked(true)}  style={{ backgroundColor: '#3f51b5', color: 'white', marginRight: '8px' }}>
            Edit
          </Button>
          {/* Assuming the View button will have functionality later */}
          <Button variant="contained" color="secondary" style={{ backgroundColor: "#f50057", color: 'white' }}>
            View
          </Button>
        </div>
      </div>
